...
#if PLATFORM_HTML5_BROWSER && defined(EMSCRIPTEN_HAS_VR_SUPPORT)
    // This is annoying -- we need to initialize VR here very early so that the Promise
    // can be fulfilled by the time we get back to it.
    emscripten_vr_init();
#endif
